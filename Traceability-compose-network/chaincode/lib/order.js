...
class OrderContract extends Contract {
    ...
    // async ConfirmAvailabilityOfProduct(ctx, rawProductName,rawProductQuantity,rawProductUnitPrice,shippingDateTime,estDeliveryDateTime) {
    ...
    async createPurchaseOrder(ctx,sellerID, fromCountry,fromState,fromCity,toCountry,toState,toCity,paymentTerms,poDateTime,productName,productQuantity,productTax,unitProductCost,packageNumber,estDeliveryDateTime,contactPersonName,contactPhoneNumber,contactEmail,packagingCost,shippingCost,totalCost) {
        ...
        // add the Purchase Order to the world state
        await ctx.stub.putState('PO_' + purchaseOrder.purchaseOrderId, Buffer.from(JSON.stringify(purchaseOrder)));
    ...
    async InsertPackagingDetails(ctx, packageDimentions, packageWeight,productId,productFragility,barCode) {
        ...
        await ctx.stub.putState('PD_' + package.packageId, Buffer.from(JSON.stringify(package)));

        const transientData = {
            barCode: barCode
        };

        await ctx.stub.putPrivateData('PackageBarCode', package.packageId, Buffer.from(JSON.stringify(transientData)));
    ...
    async OrderShipment(ctx, purchaseOrderId,batchId,batchUnitPrice,shipStartLocation,shipEndLocation,vehicleType,vehicleNumber,vehicleImage,vehicleColor,estDeliveryDateTime,gpsCoordinates,notes,status,weighbridgeSlipImage,weighbridgeSlipNumber,weighbridgeDate,tbwImage) {
       // create a new Shipment
        ...
        await ctx.stub.putState('SD_' + shipment.shipmentId, Buffer.from(JSON.stringify(shipment)));
        ...
        await ctx.stub.putPrivateData('ShipmentPrivateData', shipment.shipmentId, Buffer.from(JSON.stringify(transientData)));
